...
instance GObject TimestampArray where
    gobjectType = c_garrow_timestamp_array_get_type
    

-- | Convert 'TimestampArray' to and from 'Data.GI.Base.GValue.GValue' with 'Data.GI.Base.GValue.toGValue' and 'Data.GI.Base.GValue.fromGValue'.
instance B.GValue.IsGValue TimestampArray where
    toGValue o = do
        gtype <- c_garrow_timestamp_array_get_type
        B.ManagedPtr.withManagedPtr o (B.GValue.buildGValue gtype B.GValue.set_object)
        
    fromGValue gv = do
        ptr <- B.GValue.get_object gv :: IO (Ptr TimestampArray)
        B.ManagedPtr.newObject TimestampArray ptr
        
    

-- | Type class for types which can be safely cast to `TimestampArray`, for instance with `toTimestampArray`.
class (GObject o, O.IsDescendantOf TimestampArray o) => IsTimestampArray o
instance (GObject o, O.IsDescendantOf TimestampArray o) => IsTimestampArray o
...
instance (info ~ ResolveTimestampArrayMethod t TimestampArray, O.MethodInfo info TimestampArray p) => OL.IsLabel t (TimestampArray -> p) where
